...
namespace CSharpSampleCode
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (myclient is null)
            {
                serialPort = Logs.NovatelReader.CreateSerialPort(args);
            ...
            for (int i = 0; i < 5; i++)
            {
                try
                {
                    ...
                    if (message != null)
                    {
                        ...
                        Logs.Util.DecodeBinaryMessage(message);
                    }
        ...
        private static (TcpClient, Logs.NovatelReader) TestEthernet()
        {
            var (tcpClient, rd) = Logs.NovatelReader.CreateEthernet("192.168.1.16", 2000);
                ...
                try
                {
                    ...
                    if (line != null)
                    {
                        var (headerTokens, commandTokens) =
                            Logs.Util.TokenizeAsciiLog(line);
